...
- Snapshot below records the state at publication of the first benchmark. The current continuation is dated 2026-09-24; its latest source changes are not yet committed or pushed.
...
- The current local benchmark has ten synthetic cases, including two exact-response prompt-injection canaries; report output does not include raw messages, drafts, or canary values. Changes in this continuation add those canaries, narrow response-policy language, tests, and refreshed benchmark notes. Recheck `git status`, commit, and live remote before assuming publication.
...
- Verification: the first benchmark commit passed 110 local tests and CI. Current ten-case/two-canary continuation passes **114 backend tests with 1 upstream deprecation warning** on Python 3.11; Python syntax compilation, dataset JSON/uniqueness checks, and `git diff --check` also passed. Live Bedrock inference has not been run because no configured model/account credentials were provided. These synthetic canaries test literal marker non-disclosure only and are not evidence of general prompt-injection robustness.
