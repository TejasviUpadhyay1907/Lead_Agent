...
- Security-analysis improvement (2026-09-24): added `.github/workflows/codeql.yml` to analyze JavaScript/TypeScript and Python on pull requests, pushes to `main`, and weekly using immutable action SHAs. The workflow uses a minimal read/upload permission set. Its first GitHub run and any alert triage are pending; this is source configuration, not verified security coverage, and repository branch protection remains unchecked.
