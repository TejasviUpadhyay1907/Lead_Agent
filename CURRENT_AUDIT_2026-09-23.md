...
- Security-analysis improvement (2026-09-24): added `.github/workflows/codeql.yml` to analyze JavaScript/TypeScript and Python on pull requests, pushes to `main`, and weekly using immutable action SHAs. The first scan passed but opened a high-severity clear-text logging alert because the HMAC-secret helper printed the Secrets Manager ARN. The CLI now omits the ARN from stdout; the follow-up scan must verify closure. CodeQL is a useful static-analysis layer, not a complete security review, and repository branch protection remains unchecked.
