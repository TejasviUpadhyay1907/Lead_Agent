...
### WhatsApp consent record (implemented; no delivery provider yet)

Operators can record verified permission through `PUT /api/leads/{lead_id}/whatsapp-consent`. The API requires an E.164 number, timezone-aware consent time, controlled source, evidence reference, consent-text version, and explicit operator verification. It stores a tenant-keyed HMAC of the current phone number so future delivery code can ensure the consent still matches that number without duplicating the phone. The write and audit event are atomic, repeated use of identical evidence is idempotent, and privacy holds or durable opt-outs block consent capture. Human approval now stores the hash of the exact approved draft plus the approver and time; editing or rejecting clears that approval binding. Neither recorded consent nor approval sends a message.

The operator must inspect the actual consent evidence and policy text; these fields are an auditable evidence pointer and attestation, not legal validation. Meta requires opt-in permission before messaging and requires that consent clearly identify the business and describe the expected communications. Meta allows free-form service messages only during its 24-hour customer-service window; outside that window, only pre-approved templates are allowed. Because LeadRescue does not yet track a verified open service window, a future first-release sender should use a customer-approved template and validate its template category/content with the pilot owner. See Meta's [opt-in requirements](https://developers.facebook.com/documentation/business-messaging/whatsapp/getting-opt-in/) and [message/window rules](https://developers.facebook.com/documentation/business-messaging/whatsapp/messages/send-messages).

Still required for delivery: encrypted Meta credentials and account configuration; a narrowly scoped send action that checks current consent, number binding, suppression, privacy hold, approved-draft hash, and template policy immediately before sending; durable one-attempt/idempotency state that does not blindly retry ambiguous outcomes; verified webhook challenge and raw-body `X-Hub-Signature-256` validation; provider-message correlation; normalized accepted/sent/delivered/read/failed/unknown states; and real sandbox delivery and opt-out/replay tests. Meta retries webhook deliveries after failures, so webhook processing must be idempotent. See Meta's [webhook endpoint verification and signature guidance](https://developers.facebook.com/documentation/business-messaging/whatsapp/webhooks/create-webhook-endpoint/).
