...
export function AIAnalysisPanel({ lead, onAnalyze, busy }) {
    ...
    const privacyHeld = Boolean(lead.privacy_hold);
    return <section className="panel analysis-panel"><SectionHeader icon={BrainCircuit} title="AI understanding" subtitle="Extracted context, not business policy" layer="ai"><span className="layer-label ai">01 / UNDERSTAND</span></SectionHeader>
        {privacyHeld ? <div className="quiet-message"><LockKeyhole size={20} /><div><strong>AI processing is paused</strong><p>This lead record contains a privacy request and stays outside sales analysis and response drafting. A separate new inquiry should arrive as a new lead.</p></div></div> : optedOut ? <div className="quiet-message"><LockKeyhole size={20} /><div><strong>Outreach analysis is not actionable</strong><p>The customer's opt-out takes precedence over any prior analysis or draft.</p></div></div> : !lead.intent ? <EmptyState icon={BrainCircuit} title="Ready to understand this lead" description="Analyze the original message to extract intent and context, and prepare a response for your review."><button className="btn-ai" disabled={busy || isBlocked(lead)} onClick={onAnalyze}>{busy ? <BusyLabel>Analyzing lead context…</BusyLabel> : <><BrainCircuit size={16} /> Analyze lead</>}</button></EmptyState> : <>
...
export function PolicyDecisionPanel({ lead, config, clock, nextFollowup, followupsError, onRescue, busy }) {
    ...
    return <section className="panel policy-panel"><SectionHeader icon={ShieldCheck} title="System decision" subtitle="Deterministic policy · independent of AI" layer="policy"><span className="layer-label policy">02 / DECIDE</span></SectionHeader>
        {privacyHeld ? <div className="safety-policy"><LockKeyhole size={26} /><h3>Privacy request safeguard.</h3><p>Scoring and rescue stay paused for this lead record. A new sales inquiry should be recorded as a separate lead.</p><span className="badge-sub opt-out-label">Processing paused</span></div> : optedOut ? <div className="safety-policy"><LockKeyhole size={26} /><h3>Consent comes first.</h3><p>Scoring and outreach are not actionable for this lead. Any previously stored priority is not shown as a current decision.</p><span className="badge-sub opt-out-label">Policy blocked</span></div> : <>
...
export function ResponseComposer({ lead, text, onChange, editing, onEdit, onCancel, onSave, onApprove, onReject, busy, reviewed, onReview, dirty }) {
    ...
    return <section className="panel response-panel" id="response-workspace"><SectionHeader icon={UserCheck} title="Response workspace" subtitle="AI prepares the draft. You control what happens next." layer="human"><span className="layer-label human">03 / APPROVE</span></SectionHeader>
        {blocked ? <div className="blocked-composer"><LockKeyhole size={22} /><strong>{isOptedOut(lead) ? 'Outreach blocked by opt-out safeguard' : 'This lead is resolved'}</strong><p>No send, rescue or follow-up action is available.</p><button className="btn-primary" disabled><LockKeyhole size={15} /> Approval unavailable</button></div> : !lead.response_draft ? <EmptyState icon={Send} title="A thoughtful response starts with context" description="Run lead analysis to generate a draft for review. Approval does not send a message." /> : <>
            ...
            {!sent && <div className="draft-claim-warning" role="note"><strong>Verify before approval</strong><span>Check every factual claim against company records, especially pricing, availability, delivery dates, and promises. AI-generated text may be wrong.</span></div>}
